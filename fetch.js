...
async function fetchAndProcessTXTRecords(domain) {
    const url = `https://api.web3dns.net/?name=${domain}&type=TXT`;
        // alternative node: const url = `https://resolve.shakestation.io/dns-query?name=${domain}&type=TXT`;
        //     const url = `https://api.web3dns.net/?name=${domain}&type=TXT`;
...
function processTXTRecords(txtRecords) {
	...
	txtRecords.forEach(record => {
		...
		// If no separator found, skip this record
		console.warn(`No valid separator found in record: ${record}`);
		return;
	    }
